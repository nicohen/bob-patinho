...
public class MetricChart {
	...
	public BufferedImage createBufferedImage(int width, int height) {
		...
		chart.getXYPlot().addRangeMarker(
				getMarker(campaignMetric.getOptim(),
						...
						rb.getString("label.metric.optim")));
		chart.getXYPlot().addRangeMarker(
				getMarker(campaignMetric.getObjective(), Color.YELLOW,
						rb.getString("label.metric.objective")));
		chart.getXYPlot().addRangeMarker(
				getMarker(campaignMetric.getMinimum(), Color.ORANGE,
						rb.getString("label.metric.minimum")));
		chart.getXYPlot().addRangeMarker(
				getMarker(campaignMetric.getUnsatisfactory(), Color.RED,
						rb.getString("label.metric.unsatisfactory")));
	...
	private TimeSeriesCollection getDataSet() {
		...
		System.out.println("Este es el dia "
				+ DateUtils.getLastDay(period).getDate());
		int dayTo = 0;
...
*/
		...
		System.out.println(linearFit[0]+" "+linearFit[1]);
		double startY = getY(linearFit[0], linearFit[1], DateUtils.getFirstDay(period));
		endY = getY(linearFit[0], linearFit[1], DateUtils.getLastDay(period));
	...
	private double[] getLinearFit(SortedSet<Day> acumDays, TimeSeries pop) {
		...
		while (it.hasNext()) {
			...
			System.out.println(xValues[i-1]+" "+yValues[i-1]);
		}
