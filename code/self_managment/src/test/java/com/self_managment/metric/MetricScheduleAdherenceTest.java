...
public class MetricScheduleAdherenceTest extends TestCase {

	private MetricStrategy metric;
	private STSService stsService;

	public static Test suite() {
		return new TestSuite(MetricScheduleAdherenceTest.class);
	}
	...
	protected void setUp() throws Exception {
		...
		metric = (MetricStrategy) appContext.getBean("SCHED_ADG");
	...
	public void testMetricWithResult() throws ParseException {
		...
		assertEquals(stsService.getScheduledAdherence(null, null, 100,
				dateFrom, dateTo), metric.execute(null, null, 100, dateFrom,
				dateTo));
	}

	public void testMetricWithoutResult() throws ParseException {
		Date date = new SimpleDateFormat("dd/MM/yyyy").parse("10/10/2010");
		assertEquals(0D, metric.execute(null, null, -1, DateUtils
				.getFirstDay(date), DateUtils.getLastDay(date)));
	}
}
